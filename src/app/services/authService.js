...
// Crear instancia de axios con configuración bas2e
...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    const isFileUpload = config?.headers?.["Content-Type"]?.includes(
      "multipart/form-data",
    );
  ...
  isAuthenticated() {
    ...
    try {
      ...
      if (isTokenExpired(token)) {
        console.log(
          "⏰ Token expirado detectado en isAuthenticated. Limpiando sesión...",
        );
        this.logout();
        return false;
      }
